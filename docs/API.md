...
- `submit`：指定版本送审；DRAFT → REVIEW；最低角色 OPERATOR。业务输入：version（送审版本）。
- `reopen`：撤回修订；REVIEW → DRAFT；最低角色 OPERATOR。业务输入：无，但必须填写 remark。
- `release`：审批发布；REVIEW → RELEASED；最低角色 ADMIN；禁止创建者自审。业务输入：无，但必须填写 remark。
- `obsolete`：废止版本；RELEASED → OBSOLETE；最低角色 ADMIN；禁止创建者自审。业务输入：reason（废止原因）。

### 部门审签 `reviews`

- `approve`：同意稿件；PENDING → APPROVED；最低角色 REVIEWER；禁止创建者自审。业务输入：comment（审核意见）。
- `reject`：拒绝稿件；PENDING → REJECTED；最低角色 REVIEWER；禁止创建者自审。业务输入：comment（拒绝原因）。
